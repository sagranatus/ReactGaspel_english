...
const FirstRoute = () => (
  <View style={[styles.scene, { backgroundColor: '#fff', paddingTop:10 }]}>
    <Text style={[styles.textStyle, {textAlign:'center', color:'#01579b'}]}>
...
const SecondRoute = () => (
  <View style={[styles.scene, { backgroundColor: '#fff', paddingTop:10 }]}>
    ...
    <Text style={{color:'#01579b', fontSize:15}}>침묵</Text> - 하느님께 우리 마음의 주파수를 맞추려는 노력으로 하느님 말씀을 듣기 전에 차분한 마음 안에서 자신을 온전히 내려놓아야 합니다.{"\n"}{"\n"}

    <Text style={{color:'#01579b', fontSize:15}}>성령청원</Text> - 하느님의 말씀은 하느님께서 이끌어 주셔야만 올바로 알아들을 수 있고 성령의 도움 없이는 거룩한 독서 자체가 불가능합니다. 반드시 성령의 도움을 청해야 합니다.{"\n"}{"\n"}

    <Text style={{color:'#01579b', fontSize:15}}>독서</Text> - 하느님의 말씀을 듣는 것으로 성경 본문이 그 자체로 무엇을 말하고 있는가에 초점을 맞춰야 합니다. 세밀한 독서와 반복적인 독서가 필요하며, 말씀을 여러 차례 침묵 속에서 읽어야 합니다.{"\n"}{"\n"}

    <Text style={{color:'#01579b', fontSize:15}}>묵상</Text> - 말씀이 바로 나에게 지금 어떤 말을 걸고자 하는지에 대해 곰곰이 생각하며 노력을 기울이는 과정으로 하느님께서 지금 나에게 하고 계시는 말씀을 듣기 위해 노력해야 한다.{"\n"}{"\n"}

    <Text style={{color:'#01579b', fontSize:15}}>기도</Text> - 하느님께서 내게 주신 말씀을 되뇌이며 자연스럽게 솔직함과 그분께 대한 신뢰 안에서 하느님께 대답하는 과정입니다.{"\n"}{"\n"}

    <Text style={{color:'#01579b', fontSize:15}}>관상</Text> - 정신과 마음의 진정한 회개를 이끌어 내는 단계로, 모든 이와 모든 것을 하느님의 눈으로 바라보며 모든 것이 하느님의 은총임을 깨닫게 해주는 하느님의 선물입니다. {"\n"}
</Text>
  </View>
);
const ThirdRoute = () => (
...
const FourthRoute = () => (
  <View style={[styles.scene, { backgroundColor: '#fff', paddingTop:10 }]}>
  <Text style={styles.textStyle}>
  <Text style={{color:'#01579b', fontSize:15}}>1) 정해진 침묵의 시간</Text> {"\n"}
...
{"\n"}{"\n"}
<Text style={{color:'#01579b', fontSize:15}}>2) 들을 귀가 있는 마음</Text>{"\n"}
급하게 해서는 안됩니다. 씨뿌리는 사람의 비유를 기억하십시오. 주님은 당신 말씀의 씨를 뿌리고 계시며 나 자신은 말씀이 떨어지는 땅입니다. 열매를 맺기 위해서는 좋은 땅이 준비되어야 합니다. 
{"\n"}{"\n"}
<Text style={{color:'#01579b', fontSize:15}}>3) 지속적인 독서</Text>{"\n"}
